...
一个 macOS 13+ 菜单栏小工具，用于轻量展示本机 Codex token 使用情况、5 小时额度状态和周额度状态。
...
统计值依赖本机日志的完整性。如果日志被清理、日志目录变化，或 Codex 日志格式变化，显示结果会相应变化。
